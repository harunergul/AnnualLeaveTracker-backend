...
public class JwtUtil {
	...
	private Claims extractAllClaims(String token) {
		return Jwts.parser().setSigningKey(appSecret.getBytes()).parseClaimsJws(token).getBody();
	}
	...
	public String generateToken(PartialUser partialUser) {
		Map<String, Object> claims = new HashMap<>();
		claims.put("isExist", partialUser.isExist());
		claims.put("user", partialUser);
		return createToken(claims, partialUser.getUsername());
	}
